// Game-source registry plus a reference in-memory implementation.
import { metaFromHeaders, parsePgn, splitPgnDatabase } from './pgn';
import type { GameMeta, GameQuery, GameRecord, GameSource } from './types';

const sources: GameSource[] = [];

export function registerGameSource(source: GameSource): void {
  if (sources.some((s) => s.id === source.id)) return;
  sources.push(source);
}

export function listGameSources(): readonly GameSource[] {
  return sources;
}

/** A GameSource built from PGN text held in memory. */
export class InMemoryPgnSource implements GameSource {
  ...
  constructor(
    ...
    this.games = splitPgnDatabase(pgnDatabase).map((pgn, i) => {
      const { headers } = parsePgn(pgn);
      return { ...metaFromHeaders(`${id}-${i}`, headers), pgn };
    });
  }

  async listGames(query: GameQuery = {}): Promise<GameMeta[]> {
    const needle = query.search?.toLowerCase();
    const filtered = needle
      ? this.games.filter((g) =>
          [g.white, g.black, g.event, g.eco].some((v) => v?.toLowerCase().includes(needle)),
        )
      : this.games;
    const offset = query.offset ?? 0;
    const limit = query.limit ?? filtered.length;
    return filtered.slice(offset, offset + limit).map(({ pgn: _pgn, ...meta }) => meta);
  }
...
// ---- Built-in classics: exercise the plumbing and feed the Board Memory drill
...
registerGameSource(
  new InMemoryPgnSource(
    'samples',
    'Classic games',
    'Five famous games bundled with the app.',
    SAMPLE_PGN,
  ),
);

// Future sources (examples):
//   registerGameSource(new LocalPgnFolderSource(...)) // user's PGN files via Electron fs
//   registerGameSource(new LichessMastersSource())    // lichess masters database API
